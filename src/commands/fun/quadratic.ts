...
export default class extends Command {
	...
	run(interaction: ChatInputCommandInteraction): Promise<InteractionResponse> {
		...
		const negativeRt = ((((-1 * x) - Math.sqrt((x * x) - (4 * xSquared * x))) / (2 * xSquared))).toFixed(2);
		// Added plus here forgot to change the minus
		const positiveRt = ((((-1 * x) + Math.sqrt((x * x) - (4 * xSquared * x))) / (2 * xSquared))).toFixed(2);
		if (negativeRt === 'NaN' || positiveRt === 'NaN') {
		...
		const responseEmbed = new EmbedBuilder()
			...
			.setColor('Random')
			.setTitle('Quadratic Formula Solver')
